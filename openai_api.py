...
def run_fit_analysis(api_key, patent_text, company_name, ir_text) -> str:
    ...
    ir_section = ir_text.strip() if ir_text.strip() else (
        "업로드된 기업자료 없음. 공개자료를 활용해 분석하세요. "
        "우선 공식 홈페이지, 제품/서비스 소개, 보도자료, 공시·기업정보, "
        "채용공고, 인증·특허·정부지원 이력 등을 확인하고, 확인된 사실과 "
        "공개자료 기반 추정을 구분하세요."
    )
...
def generate_proposal_chunks(
    ...
    제안형(6~8페이지, 5개 섹션 + 참고문헌)은 한 번의 호출로 끝까지 생성되지
