...
# 현재 GitHub 구조: 모든 파일이 루트에 평탄하게 존재
# modules/ 폴더 없이 직접 import
_HERE = Path(__file__).resolve().parent
if str(_HERE) not in sys.path:
    sys.path.insert(0, str(_HERE))

from file_parser import parse_uploaded_file
from claude_api import run_gap_analysis, stream_proposal
...
with st.sidebar:
    ...
    if api_key:
        ...
        st.markdown('<div class="warning-box">API Key를 입력해야 제안서 생성이 가능합니다.</div>', unsafe_allow_html=True)
...
with col2:
    ...
    if ir_file:
        ...
        st.markdown('<div class="info-box">IR 자료가 없으면 Claude가 공개 정보를 기반으로 추론합니다. "(추정)" 표기가 적용됩니다.</div>', unsafe_allow_html=True)
...
if not can_analyze:
    ...
    if not company_name: missing.append("수요기업명")
...
if not can_generate:
    ...
    if st.session_state.proposal_md:
        ...
        with col_d1:
            st.download_button(label="📄 Markdown 다운로드",
        ...
        with col_d2:
            try:
                ...
                st.download_button(label="📝 Word (.docx) 다운로드", data=docx_bytes,
