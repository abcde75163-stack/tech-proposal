...
def parse_markdown_table(lines: list) -> list:
    ...
    for line in lines:
        line = line.strip()
        if line.startswith("|") and not re.match(r"^\|?\s*:?-{3,}:?\s*(\|\s*:?-{3,}:?\s*)+\|?$", line):
            cells = [normalize_inline_text(c.strip()) for c in line.strip("|").split("|")]
            rows.append(cells)
    return rows


def normalize_inline_text(text: str) -> str:
    """DOCX에 넣기 좋은 형태로 Markdown 링크/공백을 정리"""
    text = re.sub(r"\[([^\]]+)\]\(([^)]+)\)", r"\1 (\2)", text)
    return text.replace("<br>", "\n").replace("<br/>", "\n").replace("<br />", "\n")


def add_formatted_runs(paragraph, text: str):
    """기본적인 Markdown 인라인 서식을 Word run으로 변환"""
    ...
    for i, row_data in enumerate(rows):
        ...
        for j, cell_text in enumerate(row_data):
            if j < num_cols:
                cell = row.cells[j]
                cell.text = ""
                for line_idx, cell_line in enumerate(str(cell_text).splitlines() or [""]):
                    para = cell.paragraphs[0] if line_idx == 0 else cell.add_paragraph()
                    add_formatted_runs(para, cell_line)
                # 첫 행은 헤더 스타일
                if i == 0:
...
def markdown_to_docx(markdown_text: str) -> bytes:
    ...
    while i < len(lines):
        ...
        # 글머리 기호
            ...
            p.paragraph_format.left_indent = Inches(0.25 if not line.startswith("  ") else 0.45)
